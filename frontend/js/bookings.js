...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderProgress(status) {
        ...
        let currentStage = 0;
        if (status === 'active') currentStage = 1;
        if (status === 'completed') currentStage = 2;
...
window.cancelBooking = async (id) => {
    if (typeof showModal === 'function') {
        ...
        if (confirm('Вы действительно хотите отменить эту бронь?')) {
            await performCancel(id);
        }
    }
}
...
async function performCancel(id) {
    try {
        ...
        if (typeof showToast === 'function') showToast('Бронь отменена/удалена');
        location.reload();
    } catch (e) {
